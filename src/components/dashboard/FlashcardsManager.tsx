...
export default function FlashcardsManager() {
  ...
  useEffect(() => {
    async function loadVocabulary() {
      try {
        ...
        if (!response.ok) {
          setMessage(
            "No fue posible cargar tu vocabulario.",
          );
  ...
  async function markCard(
    ...
    try {
      const response = await fetch(
        `/api/vocabulary/${currentEntry.id}`,
        {
          method: "PATCH",
      ...
      if (learned) {
        setMessage(
          `"${updatedEntry.word}" marcada como aprendida.`,
        );
        ...
        <>
          ...
          <section className="mx-auto mt-6 max-w-3xl">
            <button
              ...
              className="group relative flex min-h-[390px] w-full flex-col items-center justify-center overflow-hidden rounded-[2.5rem] border border-white/10 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 p-8 text-center shadow-2xl shadow-cyan-950/20 transition hover:border-cyan-400/20"
            >
            ...
            <div className="mt-4 flex justify-center">
              <button
                ...
                className="rounded-2xl bg-cyan-400/10 px-5 py-3 text-sm font-bold text-cyan-300 transition hover:bg-cyan-400/20"
              >
                🔊 Escuchar{" "}
                {
                  currentEntry.word
            ...
            <div className="mt-6 flex items-center justify-between gap-4">
              <button
                ...
                className="rounded-2xl bg-white/5 px-5 py-3 text-sm font-semibold text-slate-300 transition hover:bg-white/10 disabled:opacity-40"
              >
                ← Anterior
              </button>
              ...
              <button
                ...
                className="rounded-2xl bg-gradient-to-r from-emerald-500 to-cyan-500 px-6 py-3 text-sm font-bold text-white transition hover:-translate-y-0.5 disabled:opacity-40"
              >
                Siguiente →
              </button>
            </div>
